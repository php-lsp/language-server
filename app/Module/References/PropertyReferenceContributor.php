...
final class PropertyReferenceContributor implements ReferenceContributor
{
    ...
    public function contribute(ReferenceContext $context, ReferenceConsumer $consumer): void
    {
        ...
        $propertyName = $this->resolvePropertyName($element);
        if ($propertyName === null) {
            return;
        }

        foreach ($this->indexLookup->findByKey(PropertyAccessUsageIndexer::class) as $entry) {
    ...
    private function resolvePropertyName(?Node $element): ?string
    {
        if ($element === null) {
            return null;
        }
        ...
        if ($element instanceof Node\VarLikeIdentifier) {
            $parent = $element->getAttribute('parent');
            if ($parent instanceof Node\Expr\StaticPropertyFetch) {
                return $element->toString();
            }
            if ($parent instanceof Node\PropertyItem) {
                return $element->toString();
            }
        }
